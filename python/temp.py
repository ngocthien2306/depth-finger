...
class CamProjCalibrationParams:
    ...
    def save_calib(self, calibration_yaml_path):
        ...
        calibration_data['camera_intrinsic_matrix']['data'] = self.camera_K.flatten().tolist()
        calibration_data['camera_distortion_coefficients']['data'] = self.camera_D.flatten().tolist()
        calibration_data['projector_intrinsic_matrix']['data'] = self.projector_K.flatten().tolist()
        calibration_data['relative_rotation']['data'] = self.cam2proj_R.flatten().tolist()
        calibration_data['relative_translation']['data'] = self.cam2proj_T.flatten().tolist()
        calibration_data['F']['data'] = self.F.flatten().tolist()

        with open(calibration_yaml_path, 'w') as outfile:
            yaml.dump(calibration_data, outfile)
        

    def set_calib_(self, calib_name, calib_value):
        if calib_name == 'camera_intrinsic_matrix':
            self.camera_K = calib_value
        if calib_name == 'camera_distortion_coefficients':
            self.camera_D = calib_value
        if calib_name == 'projector_intrinsic_matrix':
            self.projector_K = calib_value
        if calib_name == 'relative_rotation':
            self.cam2proj_R = calib_value
        if calib_name == 'relative_translation':
            self.cam2proj_T = calib_value
        if calib_name == 'F':
            self.F = calib_value
    
    def get_calib_(self, calib_name):
        if calib_name == 'camera_intrinsic_matrix':
            return self.camera_K
        if calib_name == 'camera_distortion_coefficients':
            return self.camera_D
        if calib_name == 'projector_intrinsic_matrix':
            return self.projector_K
        if calib_name == 'relative_rotation':
            return self.cam2proj_R
        if calib_name == 'relative_translation':
