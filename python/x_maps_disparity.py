...
# def compute_disparity(xcr_i16, ycr_i16, t, proj_x_map, T_PX_SCALE, X_OFFSET):
#     # events may not be completely sorted by time if they were processed in a filter that averages over coordinates
#     min_t = t.min()
#     max_t = t.max()
...
#     # spurious events from scene movement or noise may lie outside the projector X-map
#     # these events will be filtered out
#     y_inlier_mask = (ycr_i16 >= 0) & (ycr_i16 < proj_x_map.shape[0] - 1)
...
#         # Define an offset to differentiate between x=0 and x undefined
#         self.X_OFFSET = 4242

#         # We don't overflow with 16-bit indices
#         assert proj_time_map_rect.shape[0] <= 2**15 - 1
#         assert proj_time_map_rect.shape[1] + self.X_OFFSET <= 2**15 - 1

#         # The time axis can be freely discretized, we choose the projector width as the number of time steps
#         self.X_MAP_WIDTH = self.calib_params.projector_width
#         self.T_PX_SCALE = self.X_MAP_WIDTH - 1

#         # Define the number of threads per block and blocks per grid for GPU kernel launch
#         threads_per_block = 128
#         blocks_per_grid = (proj_time_map_rect.size + threads_per_block - 1) // threads_per_block

#         # Allocate space for the result arrays
#         self.proj_x_map = np.zeros((proj_time_map_rect.shape[0], self.X_MAP_WIDTH), dtype=np.float32)
#         t_diffs = np.zeros(proj_time_map_rect.shape, dtype=np.float32)

#         # Launch the GPU kernel for X-map computation
#         compute_x_map_from_time_map[blocks_per_grid, threads_per_block](
#             proj_time_map_rect,
#             self.X_MAP_WIDTH,
#             self.T_PX_SCALE,
#             self.X_OFFSET,
#             self.calib_params.projector_width,
#             self.proj_x_map,
#             t_diffs
#         )
...
#         ev_y_rect_i16,
#     ):
#         """Computes the disparity for events based on projector X-map."""
#         ev_disparity_f32, inlier_mask = compute_disparity(
#             ev_x_rect_i16, ev_y_rect_i16, events["t"], self.proj_x_map, self.T_PX_SCALE, self.X_OFFSET
#         )
#         return ev_disparity_f32, inlier_mask
