...
class HandDetector:
    ...
    def load_model(self, cuda: bool = False):
        if cuda:
            ...
            return ort.InferenceSession(self.path_model, providers=["CUDAExecutionProvider"])
    ...
    def postprocess(self, outputs, image, shape, original_frame):
        ...
        for output in outputs:
            ...
            for box in box_output:
                ...
                #     2,
                # )
            # [(int(k[0]), int(k[1])) for k in kps_output]) 
        ...
        tracked_objects = kt.update(keypoints_list)

    # Loop over the tracked objects
        for (objectID, keypoints) in tracked_objects.items():
            # Draw the object ID near the first keypoint
            if keypoints:
                text = f"ID {objectID}"
                print(keypoints)
                kp_x, kp_y = keypoints[0] # Use the first keypoint as a reference
                cv2.putText(original_frame, text, (kp_x - 10, kp_y - 10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 245, 255), 2)
...
#        outputs = non_max_suppression(outputs, 0.5, 0.5, 1)
        outputs = non_max_suppression(outputs, 0.3, 0.35, 1)
...
if __name__ == "__main__":
    hand_detector = HandDetector(r"best38cm.onnx", 320, 32,cuda=True)
    video = cv2.VideoCapture("case3.wmv")
